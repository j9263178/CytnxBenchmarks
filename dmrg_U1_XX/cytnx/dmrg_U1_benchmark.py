...
class Hxx(cytnx.LinOp):
    ...
    def matvec(self, v):
        ...
        if(v.shape()==[targetD, 2,2, targetD]):
            HAA_time.append(end-start)
        return out

def optimize_psi(psivec, functArgs, maxit=3, krydim=4):
    ...
    st = time.time()
    energy, psivec = cytnx.linalg.Lanczos_Gnd_Ut(H, Tin=psivec, maxiter = maxit, CvgCrit = 1.0e-12)
    #energy ,psivec = cytnx.linalg.Lanczos_Gnd(H,CvgCrit=1.0e-12,Tin=psivec,maxiter=4000)
    
    en  = time.time()

    if(psivec.shape()==[targetD, 2,2, targetD]):
...
maxit = 3 # iterations of Lanczos method
...
for k in range(1, numsweeps+2):
    ...
    for p in range(Nsites-2,-1,-1): 
        ...
        en = time.time()
        if(psi.shape()==[targetD, 2,2, targetD]):
