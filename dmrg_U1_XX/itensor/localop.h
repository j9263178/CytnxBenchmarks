...
//
#ifndef __myITENSOR_LOCAL_OP
#define __myITENSOR_LOCAL_OP
...
struct timespec t_start_, t_end_;

struct timespec diff_(struct timespec start, struct timespec end) {
...
if (printtensor)
{
        clock_gettime(CLOCK_MONOTONIC, &t_end_);

        // 計算實際花費時間
        auto temp = diff(t_start_, t_end_);
        // auto dt = double(t2 - t1) / CLOCKS_PER_SEC;
        cout << "H|psi> time (sec) = " << (double) temp.tv_nsec / 1000000000.0 << endl;
