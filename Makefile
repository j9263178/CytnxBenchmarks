...
#	$(CC) $(CYTNX_CXXFLAGS) -I$(BENCHMARK_HEADER) -I${CYTNX_INC}  $< -L$(BENCHMARK_SRC) ${CYTNX_LIB} ${CYTNX_LINK} -I$(ITENSOR_ROOT) -O2 -DNDEBUG -Wall -L$(ITENSOR_LIBDIR) -litensor -lbenchmark -fsanitize=address -o $@
	$(CC) $(CYTNX_CXXFLAGS) -I$(BENCHMARK_HEADER) -I${CYTNX_INC}  $< -L$(BENCHMARK_SRC) ${CYTNX_LIB} ${CYTNX_LINK} -I$(ITENSOR_ROOT) -O2 -DNDEBUG -Wall -L$(ITENSOR_LIBDIR) -litensor -lbenchmark -o $@
#clean
.phony: clean
