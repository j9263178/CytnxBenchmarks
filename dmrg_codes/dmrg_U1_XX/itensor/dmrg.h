...
//
#ifndef __myITENSOR_DMRG_H
#define __myITENSOR_DMRG_H
...
struct timespec t_start__, t_end__;

struct timespec diff__(struct timespec start, struct timespec end) {
...
dmrg(MPS & psi, 
     ...
     Args const& args = Args::global())
    {
    my::LocalMPO PH(H,args);
    Real energy = my::DMRGWorker(psi,PH,sweeps,args);
    return energy;
    }
...
dmrg(MPO const& H,
    ...
    {
    auto psi = psi0;
    auto energy = my::dmrg(psi,H,sweeps,args);
...
dmrg(MPS& psi, 
    ...
    {
    my::LocalMPO PH(H,args);
    Real energy = my::DMRGWorker(psi,PH,sweeps,obs,args);
    return energy;
    }
...
dmrg(MPO const& H,
    ...
    {
    auto psi = psi0;
    auto energy = my::dmrg(psi,H,sweeps,obs,args);
    return std::tuple<Real,MPS>(energy,psi);
    }
...
dmrg(MPO const& H,
    ...
    {
    auto psi = psi0;
    auto energy = my::dmrg(psi,H,LH,RH,sweeps,args);
    return std::tuple<Real,MPS>(energy,psi);
    }
...
dmrg(MPO const& H,
    ...
    {
    auto psi = psi0;
    auto energy = my::dmrg(psi,H,LH,RH,sweeps,obs,args);
    return std::tuple<Real,MPS>(energy,psi);
    }
...
dmrg(std::vector<MPO> const& Hset,
    ...
    {
    auto psi = psi0;
    auto energy = my::dmrg(psi,Hset,sweeps,args);
    return std::tuple<Real,MPS>(energy,psi);
    }
...
dmrg(std::vector<MPO> const& Hset,
    ...
    {
    auto psi = psi0;
    auto energy = my::dmrg(psi,Hset,sweeps,obs,args);
    return std::tuple<Real,MPS>(energy,psi);
    }
...
// Weight - real number w > 0; calling my::dmrg(psi,H,psis,sweeps,Args("Weight",w))
...
dmrg(MPO const& H,
    ...
    {
    auto psi = psi0;
    auto energy = my::dmrg(psi,H,psis,sweeps,args);
    return std::tuple<Real,MPS>(energy,psi);
    }
...
dmrg(MPO const& H,
    ...
    {
    auto psi = psi0;
    auto energy = my::dmrg(psi,H,psis,sweeps,obs,args);
    return std::tuple<Real,MPS>(energy,psi);
    }
...
if (b == N/2)
{
        ...
        clock_gettime(CLOCK_MONOTONIC, &t_start__);
        energy = davidson(PH,phi,args);
        clock_gettime(CLOCK_MONOTONIC, &t_end__);
        auto temp = diff__(t_start__, t_end__);
        cout << "============= Davison: =============" << endl;
        cout << "davison dim = " << maxDim(phi) << endl;
        cout << "number of davison iteration = " << ccprod << endl;
        // auto dt = double(t2 - t1) / CLOCKS_PER_SEC;
        cout << "davison time (sec) = " << (double) temp.tv_nsec / 1000000000.0 << endl;
        cout << "====================================" << endl;
